...
import { ArrowLeft, Footprints, Trash2, ScanLine } from 'lucide-react';
...
const BinNavigatePage = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 font-sans flex flex-col transition-colors duration-300">
            ...
            {/* Header */}
            <div className="bg-white dark:bg-gray-800 shadow-sm z-10 px-4 pt-6 pb-4 transition-colors duration-300">
                <div className="flex items-center gap-3">
                    <button
                        onClick={() => navigate(-1)}
                        className="p-2 -ml-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300"
                    >
                        <ArrowLeft size={24} />
                    </button>
                    <div className="flex-1 min-w-0">
                        <h1 className="text-xl font-bold text-gray-900 dark:text-white">Navigate to {bin.binName}</h1>
                        <p className="text-sm text-gray-500 dark:text-gray-400 truncate">{locationName}</p>
                    </div>
                    <ThemeToggle className="bg-gray-100 dark:bg-gray-700 mr-2 border-none" />
                    <button
                        onClick={() => navigate('/scanner')}
                        className="p-2 rounded-full bg-emerald-100 dark:bg-emerald-900/50 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-200 dark:hover:bg-emerald-800 transition-colors"
                        title="Scan QR Code"
                    >
                        <ScanLine size={20} />
                    </button>
            ...
            {/* Cards Section */}
            <div className="bg-gray-50 dark:bg-gray-900 px-4 py-4 space-y-3 flex-1 transition-colors duration-300">
                {/* Walking Route Card */}
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm flex items-center gap-4 border dark:border-gray-700 transition-colors">
                    <div className="bg-gray-100 dark:bg-gray-700 p-3 rounded-xl">
                        <Footprints size={24} className="text-gray-700 dark:text-gray-300" />
                    </div>
                    <div className="flex-1 min-w-0">
                        <p className="text-2xl font-bold text-gray-900 dark:text-white">
                            {distanceM < 1000 ? `${distanceM} m` : `${(distanceM / 1000).toFixed(1)} km`}
                        </p>
                        <p className="text-sm text-gray-600 dark:text-gray-400">{walkMinutes} min walk</p>
                        <p className="text-xs text-gray-500 dark:text-gray-500">Walking route</p>
                    </div>
                ...
                {/* Bin Status Card */}
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm flex items-start gap-4 border dark:border-gray-700 transition-colors">
                    <div className="bg-emerald-900 dark:bg-emerald-800 p-3 rounded-xl flex-shrink-0">
                        <Trash2 size={24} className="text-white" />
                    </div>
                    <div className="flex-1 min-w-0">
                        <h3 className="font-bold text-gray-900 dark:text-white text-lg">{bin.binName}</h3>
                        <div className="mt-2">
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-500 mt-2">Status: {statusText}</p>
                    </div>
                </div>

                <button
                    onClick={() => navigate('/scanner')}
                    className="w-full bg-emerald-600 text-white font-bold py-3 rounded-xl shadow-lg hover:bg-emerald-700 dark:hover:bg-emerald-500 transition-colors flex items-center justify-center gap-2"
                >
                    <ScanLine size={20} />
                    Reached location? Scan the item
                </button>
